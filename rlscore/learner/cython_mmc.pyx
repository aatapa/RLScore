...
def findSteepestDirRotateClasses(double [:, :] Y,
    ...
    for newclazz in range(labelcount):
        ...
        for h in range(takenum):
            ...
            claim_a_point(Y, R, RY, Y_Schur_RY, minus_diagRx2, classcounts, classvec, size, sqrtR, rank_R, newclazz)
...
def claim_a_point(double [:, :] Y,
                     ...
                     double [:, :] sqrtR,
                     int rank_R,
                     int newclazz):  #This function should be explicitly inlined to gain faster running speed.
