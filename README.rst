...
RLScore variables are used to refer to the different types of data inside the RLScore software.


train_features
~~~~~~~~~~~~~~

Variable containing features for training data.


train_labels
~~~~~~~~~~~~

Variable containing labels for training data. Necessary when training supervised learners.
...
Use reduced set approximation to speed up training and prediction. Restricts the learned hypothesis to be represented only by the basis vectors instead of all training data points.


folds
~~~~~

List of lists containing indices of holdout data points, one list per hold-out set. This can be used to define folds for cross-validation or, more generally, hold-out sets for repeated hold-out.
...
Variable containing features for validation data. Used by some learners for determining the point of early stopping.


validation_labels
~~~~~~~~~~~~~~~~~

Variable containing labels for validation data. Used by some learners for determining the point of early stopping.


validation_qids
~~~~~~~~~~~~~~~

Qids for the validation data. Used by some learners (e.g. CGRankRLS) for determining the point of early stopping, for query structured data.
